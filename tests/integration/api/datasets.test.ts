...
describe("POST /api/datasets", () => {
  ...
  it("rejects providing both csvText and rows with 400", async () => {
    ...
    expect(res.status).toBe(400)
    expect((await res.json()).error).toBe("provide exactly one of csvText or rows")
  })
  ...
  it("rejects a payload over 2 MB with 400 before parsing", async () => {
    ...
    expect(res.status).toBe(400)
    expect((await res.json()).error).toBe("payload too large (max 2 MB)")
  })
  ...
  it("surfaces a CSV parse failure as 400 and persists nothing", async () => {
    ...
    expect(res.status).toBe(400)
    expect((await res.json()).error).toContain("duplicate column")
